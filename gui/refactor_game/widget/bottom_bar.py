...
class BottomBar(tk.Frame):
    ...
    def __init__(self, parent,
        ...
        # input action
        self.input_action_entry = tk.Entry(self, state=tk.DISABLED)
        ...
        # pause
        self.pause_button = tk.Button(self, text='Pause', command=self.parent.observed_logical_state.handle_pause_callback, state=tk.DISABLED)
        self.pause_button.grid(row=0, column=2)

        # resume
        self.resume_button = tk.Button(self, text='Resume', command=self.parent.observed_logical_state.handle_resume_callback, state=tk.DISABLED)
        self.resume_button.grid(row=0, column=3)

        # undo last move
        self.undo_last_move_button = tk.Button(self, text='Undo Last Move', command=self.parent.observed_logical_state.handle_undo_last_move_callback, state=tk.DISABLED)
        self.undo_last_move_button.grid(row=0, column=4)

        # reset
        self.reset_button = tk.Button(self, text='Reset', command=self.parent.observed_logical_state.handle_reset_callback, state=tk.DISABLED)
